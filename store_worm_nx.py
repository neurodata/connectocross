...
for file in graphs:
    ...
    if file == 'nerve&ganglion': #nerve&ganglion does not have separate hermaphrodite and male fields
        for i, graph in enumerate(filegraphs[0]):
            ...
            file_path = os.path.join(script_dir, "worm_wiring/graphs/%s/Adult/%s.json" % (file, i))
            os.makedirs(os.path.dirname(file_path), exist_ok=True)
            with open(file_path, 'w') as f:
                json.dump(data, f)
        for i, graph in enumerate(filegraphs[1]):
            data = jg.node_link_data(graph)
            file_path = os.path.join(script_dir, "worm_wiring/graphs/%s/L4/%s.json" % (file, i))
            os.makedirs(os.path.dirname(file_path), exist_ok=True)
            with open(file_path, 'w') as f:
                json.dump(data, f)
    else:
        for i in range(len(filegraphs)):
            ...
            for j, graph in enumerate(filegraphs[i]):
                data = jg.node_link_data(graph)
                file_path = os.path.join(script_dir, "worm_wiring/graphs/%s/%s/%s.json" % (file, sex, j))
                os.makedirs(os.path.dirname(file_path), exist_ok=True)
                with open(file_path, 'w') as f:
                    json.dump(data, f)
